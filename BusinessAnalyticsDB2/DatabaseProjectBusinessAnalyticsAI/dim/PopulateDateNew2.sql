-- 
-- 2025.02  Jesper Harder       Changed to handle ISO8601 Danish/Nordic Week Numbering
--

CREATE   PROCEDURE [dim].[PopulateDateNew2]
(
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            DECLARE @YearForWeekNo nvarchar(4);
            ...
            BEGIN
                ...
                -- 2025.02 Determine the correct ISO year for the given date
                SET @YearForWeekNo = CASE 
                    WHEN MONTH(@p_date) = 1 AND DATEPART(ISO_WEEK, @p_date) >= 52 THEN YEAR(@p_date) - 1 
                    WHEN MONTH(@p_date) = 12 AND DATEPART(ISO_WEEK, @p_date) = 1 THEN YEAR(@p_date) + 1
                    ELSE YEAR(@p_date) 
                END;
                ...
                -- 2025.02 Get ISO week number (Nordic Standard)
                SET @Week = FORMAT(DATEPART(ISO_WEEK, @p_date), '00');
                ...
                -- 2025.02 xCorrected year-week format
                -- Construct the YearWeek format (yyyyWww)
                SET @YearWeek = CONCAT(@YearForWeekNo, 'W', @Week);
                --SET @YearWeek = FORMAT(@p_date, 'yyyy', 'da-DK') + N'/W' + FORMAT(@p_date, 'ww', 'da-DK');
                --SET @YearWeek = CONCAT(FORMAT(@p_date, N'yyyy/W', N'en-US'), DATEPART(wk, @p_date) - 1);

                -- 2025.025 Ensure correct ISO week number
                -- Correct ISO 8601 week number (always two digits)
                SET @WeekNo = FORMAT(DATEPART(ISO_WEEK, @p_date), '00');
                --SET @WeekNo = FORMAT(@p_date, 'ww', 'da-DK');
