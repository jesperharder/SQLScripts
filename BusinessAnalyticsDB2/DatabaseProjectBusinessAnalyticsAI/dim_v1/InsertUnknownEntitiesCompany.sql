/* This file was generated automatically by a T4 Template */

CREATE PROCEDURE [dim_v1].[InsertUnknownEntitiesCompany]
AS
BEGIN
    SET NOCOUNT ON;
 
    BEGIN TRY
    -- Get the current transaction count
    DECLARE @TranCounter INT = @@TRANCOUNT
		,@SavePoint NVARCHAR(32) = CAST(@@PROCID AS NVARCHAR(20)) + N'_' + CAST(@@NESTLEVEL AS NVARCHAR(2));

    -- Decide to join existing transaction or start new
    IF @TranCounter > 0
      SAVE TRANSACTION @SavePoint;
    ELSE
      BEGIN TRANSACTION;

    
	 
	MERGE [dim_v1].[Company] AS d
    USING (
			SELECT -1 AS [CompanyKey]
			, -1 AS [CompanyID]
			, 'Unknown' AS [CompanyNameShort]
			, 'Unknown' AS [CompanyNameLong]
			, 'Unknown' AS [CompanyCountryCode]
			, 'Unknown' AS [CompanyHashKey]
			, -1 AS [ADF_BatchId_Insert]
			, -1 AS [ADF_BatchId_Update]
			
			UNION ALL

			SELECT -2 AS [CompanyKey]
			, -2 AS [CompanyID]
			, 'Blank' AS [CompanyNameShort]
			, 'Blank' AS [CompanyNameLong]
			, 'Blank' AS [CompanyCountryCode]
			, 'Blank' AS [CompanyHashKey]
			, -2 AS [ADF_BatchId_Insert]
			, -2 AS [ADF_BatchId_Update]
			
		  ) s
	ON s.[CompanyKey] = d.[CompanyKey]
	
	WHEN NOT MATCHED THEN -- Insert new records
    INSERT ([CompanyKey]
	, [CompanyID]
	, [CompanyNameShort]
	, [CompanyNameLong]
	, [CompanyCountryCode]
	, [CompanyHashKey]
	, [ADF_BatchId_Insert]
	, [ADF_BatchId_Update]
	
	)
    VALUES (s.[CompanyKey]
		, s.[CompanyID]
		, s.[CompanyNameShort]
		, s.[CompanyNameLong]
		, s.[CompanyCountryCode]
		, s.[CompanyHashKey]
		, s.[ADF_BatchId_Insert]
		, s.[ADF_BatchId_Update]
		)
	WHEN MATCHED
	THEN      						  -- update record with update flag
        UPDATE SET
		d.[CompanyID] = s.[CompanyID]
		, d.[CompanyNameShort] = s.[CompanyNameShort]
		, d.[CompanyNameLong] = s.[CompanyNameLong]
		, d.[CompanyCountryCode] = s.[CompanyCountryCode]
		, d.[CompanyHashKey] = s.[CompanyHashKey]
		, d.[ADF_BatchId_Insert] = s.[ADF_BatchId_Insert]
		, d.[ADF_BatchId_Update] = s.[ADF_BatchId_Update]
		

	OPTION ( RECOMPILE );
 	
    
    -- Commit only if the transaction was started in this procedure
        IF @TranCounter = 0
            COMMIT TRANSACTION;
    END TRY
 
    BEGIN CATCH
 
    -- Only rollback if transaction was started in this procedure
        IF @TranCounter = 0
            ROLLBACK TRANSACTION;
    -- It's not our transaction but it's still OK
        ELSE
            IF XACT_STATE() = 1
                ROLLBACK TRANSACTION @SavePoint
    -- All hope is lost - rollback!
            ELSE
                IF XACT_STATE() = -1
                    ROLLBACK TRANSACTION;
 
        THROW;
        RETURN 1;
 
    END CATCH;
END

GO

